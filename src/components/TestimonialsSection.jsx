...
export default function TestimonialsSection() {
  ...
  return (
    <section className="bg-black py-10 sm:py-16 px-4 sm:px-10">
      {/* Rounded video card */}
      <div className="relative w-full rounded-2xl overflow-hidden" style={{ aspectRatio: '16/9' }}>
        ...
        <div className="absolute left-5 sm:left-10 top-1/2 -translate-y-1/2 z-10 max-w-[55%] sm:max-w-xs">
          <p className="text-white text-sm sm:text-2xl font-light leading-snug mb-2 sm:mb-4">
            {current.quote}
          </p>
        ...
        <div className="absolute top-3 sm:top-4 right-3 sm:right-4 z-10 flex gap-2">
          <button
            onClick={togglePlay}
            className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-white/20 hover:bg-white/30 backdrop-blur-sm flex items-center justify-center transition-colors"
            aria-label={playing ? 'Pause' : 'Play'}
          >
            {playing ? (
              <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white fill-white" viewBox="0 0 24 24">
        ...
        {/* Person selector — bottom, scrollable on mobile */}
        <div className="absolute bottom-4 sm:bottom-6 left-0 right-0 z-10 flex gap-2 overflow-x-auto px-4 sm:px-0 sm:justify-center no-scrollbar">
          {testimonials.map((t, i) => (
            <button
              key={t.id}
              onClick={() => setActive(i)}
              className={`flex items-center gap-2 sm:gap-2.5 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full backdrop-blur-sm transition-all duration-300 shrink-0 ${
                i === active
                  ? 'bg-white/25 border border-white/40 scale-105'
                  : 'bg-white/10 border border-white/10 hover:bg-white/20'
              }`}
            >
              <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-white/30 overflow-hidden flex-shrink-0">
                {t.avatar
              ...
              <div className="text-left">
                <p className="text-white text-[11px] sm:text-xs font-medium leading-tight">{t.name}</p>
                <p className="text-white/50 text-[9px] sm:text-[10px] leading-tight">{t.company}</p>
              </div>
            </button>
          ))}
        </div>
